...
export default function Home() {
  ...
  const videoUrl = "https://cdn.zupernova.dev/olaworld/hls/hero_2160p.m3u8";

  // Videos de prueba alternativos (comentados):
    ...
    <main className="relative w-full h-screen overflow-hidden">
      ...
      <div className="relative z-20 flex flex-col items-center justify-center w-full h-full pb-24 sm:pb-20">
        ...
        <div className="w-64 sm:w-80 md:w-96 lg:w-[32rem] xl:w-[40rem] max-w-[90vw] px-4 sm:px-6 md:px-8">
          <Image src="/olaworld-hero.svg" alt="Ola World" width={1920} height={1080} priority className="w-full h-auto drop-shadow-2xl" />
        </div>

        {/* Texto "Próximamente | Coming soon" */}
        <h1 className="font-bold text-white text-lg sm:text-xl md:text-2xl lg:text-3xl mt-4 md:mt-6 text-center px-4">Próximamente | Coming soon</h1>
      </div>

      {/* Footer */}
      <footer className="absolute bottom-0 w-full bg-transparent z-20">
        <p className="text-xs sm:text-sm text-white/70 text-center py-5 sm:py-4 md:py-6 w-full">©2025 Ola World, some rights reserved.</p>
      </footer>
    </main>
  );
}
